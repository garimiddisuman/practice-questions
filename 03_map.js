/*----- map a list of students' grades in multiple subjects to their -----------
---- average score, and then sort the averages in descending order in --------*/
...
const sortStudentsByAverage = function (students) {
  const avg = students.map(({ name, grades }) => ({
    name,
    avg: average(grades),
  }));

  return avg.sort((x, y) => y.avg - x.avg);
};
...
const count = function (item, strings) {
  return strings.reduce((element, count) => {
    element === item ? count + 1 : count;
  }, 0);
};
